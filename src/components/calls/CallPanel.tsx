...
function MicBanner({ permission }: { permission: string }) {
  ...
  if (permission === 'denied') {
    return (
      <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-2 text-sm text-yellow-700">
        🔇 Mic denied — call will not be recorded
      </div>
    );
  }
  if (permission === 'requesting') {
    return (
      <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2 text-sm text-blue-600">
...
export default function CallPanel() {
  ...
  const { micPermission, isRecording, requestMicAndStart, stopAndUpload, cancelRecording } = useMediaRecorder();
  ...
  // ─── Start: initiate session first, then request mic ──────────────────
  // Mic request happens AFTER session is confirmed so we never record
  // without a valid billing session already open.
  const handleStart = async () => {
    setEndedSession(null);
    const session = await initiateCall();
    if (session && user) {
      // Non-blocking: mic failure doesn't block the call
      requestMicAndStart(session.id, user.id);
    }
  };

  // ─── End: stop recording → upload → end call with path ──────────────
  ...
  const isEndingCall = isLoading || isUploading;
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        {activeSession ? (
          <div className="flex flex-col gap-5">
            {/* Mic status banner */}
            <MicBanner permission={micPermission} />
            ...
            >
              {isEndingCall ? (
                <>
                  ...
                  {isUploading ? 'Saving recording…' : 'Ending call…'}
                </>
              ) : (
                <>
                  <span className="text-xl">📵</span>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-blue-50 rounded-xl p-4 flex items-center gap-3 text-sm text-blue-600">
              <span>🎤</span>
              <span>Microphone access will be requested when you start a call</span>
            </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">
                        {session.duration_seconds !== undefined ? formatDuration(session.duration_seconds) : 'In progress'}
                      </p>
